...
func indexHandler(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != "/" {
		...
		tmpl := template.Must(template.ParseFiles("assets/templates/404.html"))
		tmpl.Execute(w, nil)
		return
		// RenderPage(w, http.StatusNotFound, "templates/404.html", struct {
		// 	error string
		// }{
		// 	error: "404 Not Found",
		// })
...
func artistHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !(id >= 0 && id <= 51) {
		...
		tmpl := template.Must(template.ParseFiles("assets/templates/400.html"))
		tmpl.Execute(w, nil)
		return
	}
	...
	if err != nil {
		...
		// })
		// return
		tmpl := template.Must(template.ParseFiles("assets/templates/500.html"))
		tmpl.Execute(w, nil)
...
func searchHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, artist := range Artists {
		if strings.Contains(strings.ToLower(query),strings.ToLower(artist.Name)) ||
			strings.Contains(strings.ToLower(query), strings.ToLower(artist.FirstAlbum)) ||
			containsMember(artist.Members, query) || strings.Contains(strings.ToLower(query), strconv.Itoa(artist.CreationDate)) {
		...
		for i := 0; i < len(finalLocs); i++ {
			if strings.Contains(strings.ToLower(query), strings.ToLower(finalLocs[i])) {
				if !checkFilteredArtists(filteredArtists, artist) {
					fmt.Println("before forRender")
	...
	var forRender []ArtistRender
	for i := 0; i < len(filteredArtists); i++ {
		artist := MakeArtistRender(filteredArtists[i].ID - 1)
		fmt.Println("forRender")
		fmt.Println(artist.Name)
		forRender = append(forRender, artist)
	}
	RenderPage(w, http.StatusOK, "index.html", forRender)
}
